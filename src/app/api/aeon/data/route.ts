/**
 * POST /api/aeon/data — data management operations.
 * Body: { action: "clearLogs" | "clearCycles" | "clearChat" | "clearActions" | "reseed" }
 */
...
export async function POST(req: Request) {
  ...
  switch (action) {
    case "clearLogs":
      ...
      return NextResponse.json({ ok: true, cleared: "logs" });
    case "clearCycles":
      await db.cycleHistory.deleteMany({});
      return NextResponse.json({ ok: true, cleared: "cycles" });
    case "clearChat":
      await db.chatTurn.deleteMany({});
      return NextResponse.json({ ok: true, cleared: "chat" });
    case "clearActions":
      await db.action.deleteMany({});
      return NextResponse.json({ ok: true, cleared: "actions" });
    case "clearMessages":
      await db.agentMessage.deleteMany({});
      return NextResponse.json({ ok: true, cleared: "messages" });
    case "reseed": {
      // Full reseed (same as /api/aeon/seed POST)
      ...
      for (const [from, to, rel] of EDGES) {
        ...
        if (fromId && toId) await db.edge.create({ data: { fromId, toId, relation: rel, weight: 1 } });
      }
      ...
      return NextResponse.json({ ok: true, reseeded: `agents: ${AGENTS.length}, devices: ${DEVICES.length}, nodes: ${NODES.length}, triggers: ${trigs}` });
    }
    default:
